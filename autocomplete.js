export class Trie {
  ...
  insert(word) {
    ...
    subTree['ENDS_HERE'] = true;
  }

  findAll(s) {
    let dict = this.root;
    for (let i = 0; i < s.length; i++) {
      // let ch1 = s.slice(0, i + 1);
      let ch1 = s[i];
      if (dict[ch1]) {
        dict = JSON.parse(JSON.stringify(dict[ch1]));
      }
    }
    this.prefix = s;
    return this.getAvailableWords(dict);
  }

  getAvailableWords(dictionary) {
    const chars = Object.keys(dictionary);
    let subresult = [];
    let result = [];
    for (let i = 0; i < chars.length; i++) {
      if (chars[i] === 'ENDS_HERE') {
        return [''];
      }
     const words = this.getAvailableWords(dictionary[chars[i]]);
     for(let j=0;j<words.length;j++){
       const value = `${chars[i]}${words[j]}`
